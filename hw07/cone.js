export class Cone {
  ...
  constructor(gl, segments = 32, options = {}) {
    ...
    for (let i = 0; i < segments; i++) {
      ...
      const bot_left_x = radius * Math.cos(angle0);
      const bot_left_z = radius * Math.sin(angle0);
      const bot_right_x = radius * Math.cos(angle1);
      const bot_right_z = radius * Math.sin(angle1);
      ...
      const [v1x, v1y, v1z] = [-bot_left_x, -1, -bot_left_z];
      const [v2x, v2y, v2z] = [-bot_right_x, -1, -bot_right_z];

      const nxt = v1y * v2z - v1z * v2y;
      const nyt = v1z * v2x - v1x * v2x;
      const nzt = v1x * v2y - v1y * v2x;

      const nsize = Math.sqrt(
        Math.pow(nxt, 2) + Math.pow(nyt, 2) + Math.pow(nzt, 2)
      );
